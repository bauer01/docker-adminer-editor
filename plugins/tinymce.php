...
tinyMCE.init({
	...
	mode: 'none',
    plugins: [
        'lists preview code fullscreen contextmenu paste'
    ],
...
<?php
	...
	function editInput($table, $field, $attrs, $value) {
		if (preg_match("~text~", $field["type"]) && preg_match("~_html~", $field["field"])) {
			return "<textarea$attrs id='fields-" . h($field["field"]) . "' rows='12' cols='50' class='tinymce'>" . h($value) . "</textarea><script type='text/javascript'>
